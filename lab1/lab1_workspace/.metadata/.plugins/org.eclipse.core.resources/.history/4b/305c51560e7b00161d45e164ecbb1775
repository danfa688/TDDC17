...
class MyAgentProgram implements AgentProgram {
	...
	private Vector<Integer> findNext(int goal){
		...
		while(!frontier.isEmpty()){
			...
			for(int i=0; i<4; i++){
				addToFrontier = true;
				if(possibleAction(i,node.xPos,node.yPos,node.direction)){
	...
	private boolean possibleAction(int action, int xpos, int ypos, int dir){
		switch(action){
		case 0:	//Forward
	...
	public Action execute(Percept percept) {
		...
		if (home && goingHome)
		{
			...
			if ((!MapSizeKnown) && (mode == 0)){
				return findMapSize(bump);
			}else{
				if ((!MapSizeKnown) && (mode == 1)){
					updateWalls();
					MapSizeKnown=true;	
				}
